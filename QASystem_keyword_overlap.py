...
def extract_question_keyword(question):
    '''

    :param question: dictionary, key: question number; value: list of questions
    :return: question keyword, key: question number; value: list of keywords in questions
    '''
    new_question = copy.deepcopy(question)
    question_keyword = {}
    question_type = ["What", "Where", "When", "Who", "How many", "Why", "name", "Name"]
    for num in new_question:
        for type in question_type:
            if type in new_question[num][0]:
                new_question[num][0].remove(type)
        question_keyword[num] = new_question[num][0]
    ...
    with open(filename, 'r', encoding='latin-1') as f:
        ...
        try:
            ...
            for sent in sentences:
                ...
                current_tokens = sent.split()
                filter_token = [word for word in current_tokens if not word in stop_words]
                doc.append(filter_token)
            return doc

def keyword_overlap(question_keyword, document, n):
    ...
    for i in range(0,len(document)):
        ...
        for word in document[i]:
            if word in question_keyword:
                freq += 1
        indices_with_freq[i] = freq

    sort_indices = sorted(indices_with_freq.items(), key=lambda s: s[1], reverse=True)

    num = 0
    for items in sort_indices:
        if num == n:
            break
        top_n_passages.append(document[items[0]])
        num += 1
...
if __name__ == "__main__":
    ...
    question_keyword = extract_question_keyword(question)
    ...
    for n in n_list:
        ...
        document = doc_sep_sentence(filename)
        top_n_passages = keyword_overlap(question_keyword[n], document, 15)
